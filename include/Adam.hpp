...
public:
    ...
    void update_weights(Tensor<float>& weights, Tensor<float> gradient) override;
...
private:
    ...
    float beta2;
    float epsilon;
    vector<Tensor<float>*> m; 
    vector<Tensor<float>*> v;
    vector<int> t;
};
...
void Adam::update_weights(Tensor<float>& weights, Tensor<float> gradient, int count) {
    if (t==0) {
        m = new Tensor<float>(weights.getSize(), 0.0);
        v = new Tensor<float>(weights.getSize(), 0.0);
    }
    gradient.transpose();
    m->printSize();
    gradient.printSize();
    ...
    // *v = v->scalarMultiply(beta2) + gradient.multiply(gradient).scalarMultiply(1-beta2);

    // Tensor<float> m_hat = m->scalarMultiply(1.0/(1.0 - pow(beta1, t)));
    // Tensor<float> v_hat = v->scalarMultiply(1.0/(1.0 - pow(beta2, t)));
